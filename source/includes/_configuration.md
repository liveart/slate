...
secondaryUnit ```optional``` | _added in v0.10.30_<br/> possible second units, show only if defaultUnit are set. Possible values are ```in```, ```ft```, ```cm```, ```mm```, ```m```. Read more on <a href="https://liveart.uservoice.com/knowledgebase/articles/1836322-how-to-configure-units">How to configure units?</a> | 
